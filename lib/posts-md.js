...
export async function getFileData(dir = './', id) {
  ...
  // keyword handling
  matter.attributes.keywords = matter.attributes.keywords || matter.attributes.keyword || ''
  if (typeof matter.attributes.keywords === 'string') {
    matter.attributes.keywords = matter.attributes.keywords.replace(/['"]/g, '').split(',').map(k => k.trim()).filter(k => k)
  }
  // word count
  const roundTo = 10,
    readPerMin = 200,
